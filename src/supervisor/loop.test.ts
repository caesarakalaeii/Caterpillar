...
test("a session that ended silently still has its branch tip named in the journal", async () => {
  ...
  // nothing else — no mention that the branch already carried the work. Session 7 read
  // that history, concluded the task was untouched, and re-implemented all of it; it
  // found out only when its push was refused as non-fast-forward, and a human then had
  // to choose between two independent implementations.
  //
  // The agent's own summary cannot be relied on for this: the summary that arrived was
  // generated by the supervisor precisely BECAUSE the session said nothing. But the
  // probe already looked at the branch, so `recordSession` knows the tip without asking
  // the agent anything. One sentence naming it is what session 7 needed.
  const COMMITTED = asTaskId("OUTAGE-4");
  await seedTask(COMMITTED, { sessions: 2 });
  ...
  const runner: SessionRunner = {
    run: () =>
      Promise.resolve({
        ...
        // Verbatim from `src/agent/session.ts`: what a session that died of context
        // exhaustion, a kill or a crash leaves behind.
  ...
  const deadline = Date.now() + 30_000;
  while (Date.now() < deadline) {
    journal = await pushedJournal(COMMITTED);
    if (journal !== "") break;
    await sleep(100);
  }
  ...
  assert.match(
    journal,
    /6a5db6f/,
    "the journal must name the tip, so the next session can check it against its own HEAD",
  );
  assert.match(
    journal,
    /agent\/OUTAGE-4/,
    "and the branch the tip is on, which is where the next session must look",
  );

  await retire(COMMITTED);
});
