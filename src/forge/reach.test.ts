...
test("a separator the human left out is still the same repo", () => {
  // The failure this exists for: `/brainstorm acme/allchat` against an
...
test("a repo named under the wrong owner is matched on the name", () => {
  // `acme/all-chat` for `acmecorp/all-chat` — the owner is a shorthand nobody
  ...
  assert.equal(
    nearestSlug(repo("acme/all-chat"), ["acmecorp/all-chat"]),
    "acmecorp/all-chat",
  );
});
...
test("a bare name is what a mint 422 has to work with, and it is enough", () => {
  ...
  assert.equal(nearestName("allchat", ["acme/all-chat"]), "acme/all-chat");
  assert.equal(
    nearestName("all-chat", ["acme/all-chat"]),
    undefined,
    "a name the installation does list is not the one the 422 was about",
  );
  assert.equal(nearestName("tractor", ["acme/all-chat"]), undefined);
});
...
const CATALOG = [
  ...
  "acme/all-chat",
  "acme/all-chat-extension",
  "acme/infra-deployment",
  "acme/streamer-shield",
];
...
test("a prefix beats a match in the middle of a name", () => {
  ...
  assert.equal(ranked[0], "acme/Caterpillar", ranked.join(", "));
});

test("the owner is searchable too, and case never matters", () => {
  assert.deepEqual(rankRepos("ACME/STREAMER", CATALOG), [
    "acme/streamer-shield",
  ]);
});

test("a typo still finds it, and a different word finds nothing", () => {
  assert.deepEqual(rankRepos("all-chta", CATALOG), ["acme/all-chat"]);
