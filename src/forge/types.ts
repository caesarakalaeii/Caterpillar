...
import type { RepoReach } from "./reach.ts";
 ...
 * credential can reach at all. REQUIRED rather than optional, because the paths that ask
 * (the `/brainstorm` door, intake, the claim loop) all fall back to letting the task
 * through when nobody can answer, and an implementation that quietly declined to answer
 * would restore exactly the failure this exists to remove: a mid-session `git clone` that
 * dies on a repo nobody could have reached.
 */
export interface ForgeFactory extends RepoReach {
